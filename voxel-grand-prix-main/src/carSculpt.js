...
export function bucketMaterials(profile = 'studio') {
  if (profile === 'race') {
    return [
      new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.28, metalness: 0.18, envMapIntensity: 0.70 }),
      new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.48, metalness: 0.08, envMapIntensity: 0.42 }),
...
export async function buildSculptedCar(team, h, onProgress = () => {}, profile = 'studio') {
  ...
  const bodyGrid = await sampleGrid([X0W, X1W, 0, Y1W, -Z1W, Z1W], h, material, {
    mirrorZ: true,
    onProgress: f => onProgress(0.02 + f * 0.5, `Sculpting body ${Math.round(f * 100)}%`),
  });
  onProgress(0.54, 'Painting livery and marks…');
  for (const d of (team.decals || [])) applyDecal(bodyGrid, d);
  onProgress(0.56, 'Assembling body mesh…');
  ...
  // ---- DRS flap ----
  onProgress(0.72, 'Installing DRS wing…');
  ...
  // ---- wheels ----
  onProgress(0.80, 'Forging wheels and tires…');
  const wheels = [];
  const wheelGeoCache = {};
  const defs = [
