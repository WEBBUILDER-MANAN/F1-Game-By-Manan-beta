...
export class Input {
  constructor() {
    this.keys = new Set();
    this.steeringSensitivity = Number(localStorage.getItem('voxelf1-steering-sensitivity')) || 0.85;
  ...
  setSteeringSensitivity(value) {
    this.steeringSensitivity = Math.max(0.35, Math.min(1, Number(value) || 0.85));
    localStorage.setItem('voxelf1-steering-sensitivity', String(this.steeringSensitivity));
  }
  ...
  update(dt) {
    ...
    sTarget *= this.steeringSensitivity;

    // Smooth digital steering with quick corner response and controlled return.
    const attack = 3.0, release = 4.6;
