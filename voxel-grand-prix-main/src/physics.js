...
export const CAR = {
  ...
  mu: 1.58,
  muBrake: 1.70,
  cornerStiff: 12.2,   // per rad, per N of load
  satSlip: 0.080,      // rad at grip peak
  steerMax: 0.46, steerMin: 0.075, steerFade: 0.0020,
  brakeForceMax: 42000,
};
...
export class CarPhysics {
  ...
  step(dt, input) {
    ...
    // Add a small stability pass after integration so the car does not keep
    // building a sustained side-slip tail-out drift from steering input alone.
    const sideSlip = Math.abs(this.vy) / Math.max(this.speed, 1.0);
    const sideDamp = THREE.MathUtils.clamp(1 - dt * (2.4 + sideSlip * 7.0), 0.78, 1.0);
    const yawDamp = THREE.MathUtils.clamp(1 - dt * (0.72 + sideSlip * 3.6), 0.91, 1.0);
    this.vy *= sideDamp;
