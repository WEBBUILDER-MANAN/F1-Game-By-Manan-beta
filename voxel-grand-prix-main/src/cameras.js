...
export class CameraRig {
  ...
  update(dt) {
    ...
    if (mode === 'chase') {
      ...
      // Include chassis pitch in the mount so first-person view stays attached
      // over kerbs and crests instead of lifting in world space.
      _mountF.copy(F).setY(Math.sin(p.groundPitch)).normalize();
      _target.copy(p.pos)
        .addScaledVector(_mountF, off.z)
        .addScaledVector(L, off.x)
        .add(_u.set(0, off.y, 0));
      const shake = mode === 'cockpit' ? 0 : 0.006 * t * t;
      ...
      if (q) {
        ...
        // Keep the driver's heading dominant; the track sample provides only
        // a gentle look-ahead so corners do not pull the camera sideways.
        _lookS.copy(_target).addScaledVector(_mountF, ahead);
        _lookT.lerp(_lookS, 0.16);
      } else {
        _lookT.copy(_target).addScaledVector(_mountF, ahead);
      }
      const lam2 = mode === 'cockpit' ? 18 : 13;
      this.look.x = damp(this.look.x, _lookT.x, lam2, dt);
      this.look.y = damp(this.look.y, _lookT.y, lam2, dt);
      this.look.z = damp(this.look.z, _lookT.z, lam2, dt);
      this.fov = damp(this.fov, (mode === 'cockpit' ? 71 : 60) + t * 9, 5, dt);
      // Keep the driver's horizon level; only the car's pitch changes the view.
      this.camera.up.set(0, 1, 0);
    }
...
const _mountF = new THREE.Vector3();
