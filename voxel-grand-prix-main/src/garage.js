...
const REMOTE_CAR_URL = 'https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Assets/main/Models/CarConcept/glTF-Binary/CarConcept.glb';
...
export class Garage {
  ...
  buildUI() {
    ...
    el.innerHTML = `
      ...
      <div id="garage-progress" class="hidden">
        ...
        <div id="garage-msg">Sculpting car…</div>
      </div>
      <div id="garage-chips"></div>
      <div id="garage-actions">
        <button id="garage-select">✓ Drive This Car</button>
        <button id="garage-back">Back to Main Menu</button>
      </div>
      <div id="garage-hint">Drag to rotate · Scroll to zoom · Car auto-shows DRS</div>`;
  ...
  setUnlockedTeams(ids) {
    const allowed = new Set(ids);
    this.ui.querySelectorAll('.garage-chip').forEach(chip => {
      const locked = !allowed.has(chip.dataset.id);
      chip.classList.toggle('locked', locked);
      chip.disabled = locked;
      chip.title = locked ? 'Win more Grand Prix races to unlock this car.' : '';
    });
  }
  ...
  async show(teamId) {
    ...
    if (!built) {
      ...
      try {
        built = await this.buildRemoteCar();
        msg.textContent = 'Internet showroom model loaded';
      } catch (error) {
        try {
          ...
          msg.textContent = 'Unable to load this car';
          throw fallbackError;
        }
      } finally {
    ...
    $('#garage-stats').innerHTML = st.remote
      ? 'GLB showroom asset · CC BY 4.0'
      : `Voxels <b>${st.voxels.toLocaleString()}</b> · Faces <b>${st.faces.toLocaleString()}</b> · Sculpt <b>${st.ms}ms</b>`;
    this.ring.material.color.set(team.uiColor2);
  }

  async buildRemoteCar() {
    const gltf = await this.loader.loadAsync(REMOTE_CAR_URL);
    const model = gltf.scene;
    const bounds = new THREE.Box3().setFromObject(model);
    const size = bounds.getSize(new THREE.Vector3());
    const longest = Math.max(size.x, size.z) || 1;
    model.scale.setScalar(5.2 / longest);
    ...
    model.traverse(o => {
      ...
      if (o.material) {
        ...
        materials.forEach(mat => {
          mat.envMapIntensity = Math.max(mat.envMapIntensity ?? 1, 0.8);
          if ('roughness' in mat) mat.roughness = Math.min(mat.roughness ?? 0.5, 0.42);
        });
      }
    });
