...
export class Race {
  ...
  start() {
    ...
    this._timers.push(setTimeout(() => {
      ...
      this.hud.message('Lights out!<span class="en">LIGHTS OUT!</span>', 1800, 'go');
      this.state = 'racing';
      this.raceTime = 0;
  ...
  driverInput(e) {
    ...
    const skill = e.isPlayer && this.autopilotActive ? 1.08 : (e.skill ?? 1);
    ...
    for (const o of this.entries) {
      ...
      if (!isAhead || dGap > 46 || dGap < -0.5) continue;
    ...
    const cornerLook = this.track.lineAt(q.s + Math.max(18, v * 0.72));
    const cornerClosing = cornerLook.vT < v - 5;
    if (ahead && aheadGap < engageDist && !cornerClosing) {
    ...
    const La = offRoad ? 7 + v * 0.22 : 5.5 + v * 0.32;
    const sT = q.s + La;
    ...
    const brakeLook = this.track.lineAt(q.s + Math.max(22, v * 0.82));
    let vAllow = Math.min(myLine.vT, this.track.lineAt(q.s + v * 0.28).vT, brakeLook.vT + 4) * 0.98 * skill;
    ...
    if (cornerClosing) vAllow = Math.min(vAllow, cornerLook.vT + 2.5);
    if (ahead) {
      ...
      const desired = (4.2 + 0.19 * v) * packHW;
  ...
  inputFor(e, userInput) {
    if (e.isPlayer && this.pitting) return { steer: 0, throttle: 0, brake: 1 };
  ...
  update(dt, playerInput) {
    ...
    for (const e of this.entries) {
      ...
      if (!q) continue;
      if (e.isPlayer) {
        const pitSide = this.track.cfg.pitSide || 1;
        const nearPitEntry = q.frac > 0.965 || q.frac < 0.035;
        const atPitEdge = q.lat * pitSide > this.track.width / 2 - 1.6;
        this.pitReady = !this.pitting && this.pitCooldown <= 0 && nearPitEntry && atPitEdge;
        if (this.pitReady && p.speed < 18 && p.brake > 0.45) {
          this.pitting = true;
          this.pitTimer = 4.2;
          this.pitStops++;
          p.locked = true;
          p.vx = 0; p.vy = 0; p.yawRate = 0;
          this.pitReady = false;
          this.hud.message('PIT STOP<br>Service in progress · hold brake', 1800, 'go');
        }
      }
      if (e.isPlayer && this.pitting) {
        this.pitTimer -= dt;
        p.locked = true;
        p.vx = 0; p.vy = 0; p.yawRate = 0;
        if (this.pitTimer <= 0) {
          this.pitting = false;
          this.pitCooldown = 2.5;
          p.locked = false;
          this.hud.message('PIT STOP COMPLETE', 1200, 'go');
        }
        continue;
      }
        ...
        if (e.cp[0] && e.cp[1]) {
          ...
          if (e.isPlayer) {
            ...
            if (this.mode === 'time' || e.lapCount < this.laps) {
              this.hud.message(
                `${isBest ? '<span class="best">🏁 Personal best lap!</span>' : '🏁 Lap complete'}<br><b>${msFmt(lapMs)}</b>`,
                2200, isBest ? 'purple' : ''
              );
            }
          }
  ...
  reset() {
    ...
    this.hud.message('Reset to track · lap voided', 1500);
  }
}
