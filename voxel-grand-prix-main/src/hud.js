...
export class Hud {
  constructor() {
    ...
    this.el = {
      ...
      pit: $('hud-pit'), pitCount: $('hud-pit-count'), tyres: $('hud-tyres'),
  ...
  redrawMap() {
    ...
    const pitEntry = this.track.placeAt(0.986, 0);
    const [px, pz] = this.mapFn(pitEntry.p.x, pitEntry.p.z);
    bctx.save();
    bctx.fillStyle = '#e10600';
    bctx.strokeStyle = '#ffffff';
    bctx.lineWidth = 2;
    bctx.beginPath();
    bctx.arc(px, pz, 7, 0, Math.PI * 2);
    bctx.fill();
    bctx.stroke();
    bctx.fillStyle = '#ffffff';
    bctx.font = '900 10px Arial';
    bctx.textAlign = 'center';
    bctx.textBaseline = 'middle';
    bctx.fillText('P', px, pz + 0.5);
    bctx.restore();
  }
  ...
  update(state) {
    ...
    if (corner !== this.cornerShown) {
      ...
      if (corner) {
        this.el.corner.innerHTML = `<span class="corner-en">${corner.n}</span>`;
    ...
    const pitState = race.pitting
      ? `SERVICE ${race.pitTimer.toFixed(1)}s`
      : race.pitReady && p.speed < 18 && p.brake > 0.45
        ? 'BRAKE HELD'
        : race.pitReady
          ? 'BRAKE <65 KM/H'
          : 'CLOSED';
    this.el.pit.className = `hud-pit ${race.pitting ? 'active' : (race.pitReady ? 'ready' : '')}`;
    this.el.pit.innerHTML = `PIT LANE <b>${pitState}</b>`;
    this.el.pitCount.textContent = `PIT ${race.pitStops}`;
    this.el.tyres.textContent = p.surface === 'road' || p.surface === 'kerb' ? 'ROAD' : p.surface.toUpperCase();
